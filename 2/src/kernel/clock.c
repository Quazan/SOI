...
#define PRIORITY_SCHED_RATE  (5*SCHED_RATE)  /* number of ticks per PRIORITY process*/
 ...
 */
  ...
  /*if process have priority change sched_ticks from SHED_RATE to PRIORITY_SCHED_RATE
   *and change his priority to NORMAL
   */	
  if(sched_ticks == SCHED_RATE && rp->user_priority == PRIORITY)
  {
	sched_ticks = PRIORITY_SCHED_RATE;
	rp->user_priority = NORMAL;
  }
