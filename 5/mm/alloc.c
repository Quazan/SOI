...
#define FIRST 0 /*First-fit algorithm (default) */
#define WORST 1 /*Worst-fit algorithm */
...
PRIVATE int ALGORITHM = FIRST; /* variable indicating wich algorithm is used */
 ...
 */
  ...
  if(ALGORITHM == WORST)
  {
	do {
		...
		biggest = hp = hole_head; /* biggest hole */
		...
		while (hp != NIL_HOLE && hp->h_base < swap_base) 
		{
			...
			prev_ptr = hp;
			hp = hp->h_next;
		} /*finding the best hole */

		if (biggest->h_len >= clicks) 
		{
			/* We found biggest hole.  Use it. */
			old_base = biggest->h_base;	/* remember where it started */
			biggest->h_base += clicks;	/* bite a piece off */
			biggest->h_len -= clicks;	/* ditto */
...
{
	register struct hole *it;  /* iterator */
	
	phys_clicks buffer[NR_HOLES * 2 + 1], size, i, to_copy;
	
	size = (mm_in.m1_i1 - sizeof(phys_clicks)) / (sizeof(phys_clicks)); /* max buffer size excluding 0 */
	i = 0; /* current buffer size */

	for(it = hole_head; it != NIL_HOLE && it->h_base < swap_base && i < size-1; it = it->h_next) /* iterating through all holes, until filling up all buffer*/
	{
		buffer[i++] = it->h_len;  /*adding hole length to buffer */
		buffer[i++] = it->h_base;  /* adding hole base to buffer */
	}
	buffer[i] = 0;  /* ending buffer with 0 value */
	
	to_copy = (phys_clicks) (i * sizeof(phys_clicks)); /* size to copy from buffer to user buffer */

	sys_copy(MM_PROC_NR, D, (phys_clicks) buffer, who, D, (phys_clicks) mm_in.m1_p1, to_copy); /* copying buffer to user buffer */
	
	return i/2;  /* returning quantity of holes in buffer */
}
...
PUBLIC int do_worst_fit(void)
{
  ...
  if(algorithm == FIRST)
  {
	ALGORITHM = FIRST; /* changing allocation algorithm to FIRST-FIT */ 
  }
  else if(algorithm == WORST)
  {
	ALGORITHM = WORST; /* changing allocation algorithm to WORST-FIT */
  }
  else return -1;
